...
export const createCheckEnricher = (
  ...
  const hasOtherOptions = options.tool || options.vehicle || options.format || options.passive || options.activity || options.rules
  const isMultipleSkills = Array.isArray(options.skill) && options.skill.length > 1
...
export const createSkillCheck = (skill: Skill, options?: Omit<CheckEnricherOptions, 'skill'>): string => {
  return createCheckEnricher({ ...options, skill }, 'skill')
}

export const createAbilityCheck = (ability: Ability, options?: Omit<CheckEnricherOptions, 'ability'>): string => {
...
export const createDamageRoll = (formula: string): string => {
 ...
 */
export const createAttackRoll = (options?: AttackEnricherOptions | string | number): string => {
  ...
  if (options.format && !options.formula && !options.activity && !options.attackMode && !options.rules) {
    return `[[/attack ${options.format}]]`
  }

  // Shorthand: Only formula provided (with + or - prefix)
  if (options.formula !== undefined && !options.activity && !options.attackMode && !options.format && !options.rules) {
    const formula = typeof options.formula === 'number' 
      ? `+${options.formula}` 
      : options.formula
    if (/^[+-]/.test(formula)) {
  ...
  if (options.formula !== undefined && options.attackMode && !options.activity && !options.format && !options.rules) {
    const formula = typeof options.formula === 'number' 
